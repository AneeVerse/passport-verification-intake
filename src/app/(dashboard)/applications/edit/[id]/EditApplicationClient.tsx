...
interface ApplicationData {
  ...
  application_gender: number;
  application_mobile: string;
  application_office: number;
  application_dob: string | Date;
  application_age: number;
  application_nstatus: number | null;
  application_nproof: string | null;
  application_nother: string | null;
  application_mnproof: string | null;
  application_mnother: string | null;
  application_fnproof: string | null;
  application_fnother: string | null;
  application_aproof: string;
  application_aother: string | null;
  application_dproof: string;
  application_dother: string | null;
  application_mstatus: string;
  application_mproof: string | null;
  application_addocuments: string;
  application_adother: string | null;
  application_ecnr: number;
  application_ecother: string | null;
  application_bdate: string | Date;
  application_notes: string | null;

  // new fields
  application_employment: string;
  application_student_id: string | null;
  application_reissue_reason: string | null;
  application_fir_uploaded: boolean;
  application_affidavit_confirm: boolean;
  application_mproof_option: string | null;
  application_noc_confirm: boolean;
  application_ic_confirm: boolean;
  application_name_spelling_same: boolean;
  application_name_mismatch_remark: string | null;
  application_address_same: boolean;
  application_address_remark: string | null;
...
export default function EditApplicationClient({
  ...
  const formatDate = (dateVal: string | Date | null) => {
    if (!dateVal) return "";
    return new Date(dateVal).toISOString().substring(0, 10);
  };

  // Core Form States initialized from existing record
  const [appType, setAppType] = useState(String(application.application_type));
  const [dob, setDob] = useState(formatDate(application.application_dob));
  const [age, setAge] = useState<number | "">(application.application_age);
  const [employmentType, setEmploymentType] = useState(application.application_employment || "Not Employed");
  const [maritalStatus, setMaritalStatus] = useState(application.application_mstatus || "1");
  const [scheme, setScheme] = useState(String(application.application_mode));

  // Reissue spec states
  const [reissueReason, setReissueReason] = useState(application.application_reissue_reason || "");
  const [firUploaded, setFirUploaded] = useState(application.application_fir_uploaded || false);
  const [affidavitConfirm, setAffidavitConfirm] = useState(application.application_affidavit_confirm || false);

  // Employment spec states
  const [studentId, setStudentId] = useState(application.application_student_id || "");
  const [nocConfirm, setNocConfirm] = useState(application.application_noc_confirm || false);
  const [icConfirm, setIcConfirm] = useState(application.application_ic_confirm || false);

  // Marital spec states
  const [mproofOption, setMproofOption] = useState(application.application_mproof_option || "");

  // Name & Address spelling validation states
  const [nameSame, setNameSame] = useState(application.application_name_spelling_same !== false);
  const [nameRemark, setNameRemark] = useState(application.application_name_mismatch_remark || "");
  const [addressSame, setAddressSame] = useState(application.application_address_same !== false);
  const [addressRemark, setAddressRemark] = useState(application.application_address_remark || "");

  // Checkbox Document checklists
  const [aproofSelected, setAproofSelected] = useState<string[]>(
    application.application_aproof ? application.application_aproof.split(",").filter((x) => x !== "") : []
  );
  const [dproofSelected, setDproofSelected] = useState<string[]>(
    application.application_dproof ? application.application_dproof.split(",").filter((x) => x !== "") : []
  );
  const [addocSelected, setAddocSelected] = useState<string[]>(
    application.application_addocuments ? application.application_addocuments.split(",").filter((x) => x !== "") : []
  );

  // Verification Checklist states
  const [verifyName, setVerifyName] = useState(application.application_verify_name || false);
  const [verifyDob, setVerifyDob] = useState(application.application_verify_dob || false);
  const [verifyAddress, setVerifyAddress] = useState(application.application_verify_address || false);
  const [verifyNationality, setVerifyNationality] = useState(application.application_verify_nationality || false);
  const [verifyOriginals, setVerifyOriginals] = useState(application.application_verify_originals || false);

  // Declarations
  const [criminalCase, setCriminalCase] = useState(application.application_criminal_case || false);
  ...
  const [nProof, setNProof] = useState(application.application_nproof || "");
  const [mnProof, setMnProof] = useState(application.application_mnproof || "");
  const [fnProof, setFnProof] = useState(application.application_fnproof || "");
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      try {
        const res = await updateApplication(application.application_id, formData);
        if (res.success) {
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-4 border-b border-slate-800 pb-4">
        ...
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-100">Edit Passport Intake File</h2>
          <p className="text-sm text-slate-400">Modify application details and run document checklist verification</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              ...
              <input
                ...
                required
                defaultValue={formatDate(application.application_date)}
            ...
            <div>
              ...
              <input
                ...
                required
                defaultValue={application.application_name}
            ...
            <div>
              ...
              <input
                ...
                required
                defaultValue={application.application_surname}
                placeholder="Surname"
                className="block w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            ...
            <div>
              ...
              <select
                ...
                required
                defaultValue={application.application_gender}
                className="block w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
            ...
            <div>
              ...
              <input
                ...
                required
                defaultValue={application.application_mobile}
            ...
            <div>
              ...
              <input
                ...
                defaultValue={application.application_email || ""}
            ...
            <div>
              ...
              <select
                ...
                required
                defaultValue={application.application_office}
                className="block w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                      ...
                      <div>
                        ...
                        {nProof === "-1" && (
                          <input
                            ...
                            required
                            defaultValue={application.application_nother || ""}
                        ...
                        <div>
                          ...
                          {mnProof === "-1" && (
                            <input
                              ...
                              required
                              defaultValue={application.application_mnother || ""}
                              placeholder="Specify details..."
                              className="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-lg text-slate-100 text-sm mt-2"
                            />
                          )}
                        ...
                        <div>
                          ...
                          {fnProof === "-1" && (
                            <input
                              ...
                              required
                              defaultValue={application.application_fnother || ""}
                              placeholder="Specify details..."
                              className="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-lg text-slate-100 text-sm mt-2"
                            />
                          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                required
                defaultValue={formatDate(application.application_bdate)}
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                rows={3}
                defaultValue={application.application_notes || ""}
